<template>
  <div ref="parent" class="wheel-scroll-wrapper" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" @wheel="onWheel"
    ...
    <div class="wheel-scroll-track" v-show="scrollBarVisible">
      <div class="wheel-scroll-bar" ref="bar" @mousedown="onMouseDownScrollBar" @selectstart="onSelectStartScrollBar">
...
export default {
  ...
  computed: {
    ...
    // childHeight () { //无效
    //   console.log('更新内容高度',this.$refs.child.getBoundingClientRect().height)
  ...
  methods: {
    listenToRemoteResources () {
      ...
      Array.from(tags).map((tag) => {
        if (tag.hasAttribute('data-wheel-listened')) { return }
        tag.setAttribute('data-wheel-listened', 'yes')
